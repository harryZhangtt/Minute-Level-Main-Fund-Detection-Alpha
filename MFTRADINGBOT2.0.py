...
class DataCombiner:
    ...
    #             cp_files = [f for f in os.listdir(cp_data_dir) if
    #                         os.path.isfile(os.path.join(cp_data_dir, f)) and f.startswith(cp_prefix) and f.endswith(
    #                             '.txt')]
    #             v_files = [f for f in os.listdir(v_data_dir) if
    ...
    #                     cp_data = cp_data.iloc[:min_length]
    #                     v_data = v_data.iloc[:min_length]
    #
    ...
    #
    #                     """
    #                     3.merge the price and volume data
    #                     """
    #
    #                     # Take the intersection of Date and SECU_CODE
    #                     intersected_data = pd.merge(cp_long[['Date', 'SECU_CODE','minute']], v_long[['Date', 'SECU_CODE','minute']],
    #                                                 on=['Date', 'SECU_CODE','minute'], how='inner')
    #
    #                     # Merge the price data into the intersected data
    #                     merged_df = pd.merge(intersected_data,cp_long, on=['Date', 'SECU_CODE','minute'], how='left')
    #
    #                     # Merge the volume data into the merged dataframe
    ...
    #                     ##exceed the max length,deque it
    #                         self.pv_df= self.queue_to_dataframe()
    #                         self.calculate_volume_critical()
    #                         self.merge_total_share()
    #                         self.calculate_main_fund_weighted()
    #                         # self.simple_backtest('industry_size_neutral_alpha')
    #                         self.all_results.append(self.latest_data)
    ...
    #
    #                         # Merge the price data into the intersected data
    #                         merged_df = pd.merge(intersected_data, cp_long, on=['Date', 'SECU_CODE', 'minute'],
    #                                              how='left')
    #
    #                         # Merge the volume data into the merged dataframe
    #                         merged_df = pd.merge(merged_df, v_long, on=['Date', 'SECU_CODE', 'minute'], how='left')
    ...
    #                         print(
    #                             f"Failed to process files with different encoding {cp_file_path} and {v_file_path}: {e}")
    #                 except Exception as e:
    ...
    # def calculate_main_fund_weighted(self):
    #     df = self.pv_df.copy()
    #     df = df.sort_values(by=['Date', 'SECU_CODE'])
    #
    #     # Ensure the Date column is in datetime format
    #     df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
    #
    #     # Filter the rows where volume is greater than or equal to Vcritical and create a copy
    #     time_condition = (df['minute'] > 10) & (df['minute'] < 237)
    #     df_filtered = df[(df['v'] >= df['Vcritical']) & time_condition].copy()
    #
    #     # Calculate the weighted price
    #     df_filtered['WEIGHTED_PRICE'] = df_filtered['cp'] * df_filtered['v']
    #
    #     # Sum up the weighted prices and volumes for each ticker and day
    #     df_grouped = df_filtered.groupby(['SECU_CODE', 'Date']).agg({'WEIGHTED_PRICE': 'sum', 'v': 'sum'}).reset_index()
    #
    #     # Calculate the main fund weighted price
    #     df_grouped['MAIN_FUND_WEIGHTED'] = df_grouped['WEIGHTED_PRICE'] / df_grouped['v']
    #
    #     # Set the index for alignment
    #     df_grouped.set_index(['SECU_CODE', 'Date'], inplace=True)
    #     df.set_index(['SECU_CODE', 'Date'], inplace=True)
    #
    #     # Update the original DataFrame with the calculated MAIN_FUND_WEIGHTED
    #     df.loc[df_grouped.index, 'MAIN_FUND_WEIGHTED'] = df_grouped['MAIN_FUND_WEIGHTED']
    #
    #     # Reset index to original state
    #     df.reset_index(inplace=True)
    #
    #     df['WEIGHTED_PRICE'] = df['cp'] * df['v']
    #     df['DAILY_WEIGHTED'] = (df.groupby(['Date', 'SECU_CODE'])['WEIGHTED_PRICE'].transform('sum')) / \
    #                            df.groupby(['Date', 'SECU_CODE'])['v'].transform('sum')
    #     df['VWAP'] = df['DAILY_WEIGHTED']
    #     df.drop(columns=['WEIGHTED_PRICE'], inplace=True)  # Clean up temporary column
    #
    #     # Applying the operation to calculate 'main_fund_alpha'
    #     df['main_fund_alpha'] = df.apply(
    #         lambda row: (row['MAIN_FUND_WEIGHTED'] / row['VWAP'] - 1) if row['VWAP'] != 0 else 0, axis=1)
    #
    #     self.daily_data = df.groupby(['Date', 'SECU_CODE']).last().reset_index()
    #     daily_df = self.daily_data.copy()
    #
    #     daily_df = daily_df.sort_values(by='Date')
    #     # Rank within each trading day
    #     daily_df['rank'] = daily_df.groupby('Date')['main_fund_alpha'].rank()
    #
    #     # normalized factor for today
    #     daily_df['normalized_alpha'] = daily_df.groupby('Date')['rank'].transform(
    #         lambda x: 2 * (x - 1) / (len(x) - 1) - 1 if len(x) > 1 else 0)
    #
    #     # Compute log_size
    #     daily_df['log_size'] = np.log(daily_df['TRADABLE_SHARES'] * daily_df['cp'])
    #
    #     # Compute industry neutral alpha
    #     daily_df['industry_neutral_alpha'] = daily_df['normalized_alpha'] - \
    #                                          daily_df.groupby('industry')['normalized_alpha'].transform('mean')
    #
    #     # Set industry_size_neutral_alpha to NaN initially
    #     daily_df['industry_size_neutral_alpha'] = np.nan
    #
    #     def calculate_residuals(group, vector_column, response_column):
    ...
    #         return alpha
    #
    #     # Calculate industry size neutral alpha only for the latest date
    #     latest_date = daily_df['Date'].max()
    #     latest_data = daily_df[daily_df['Date'] == latest_date].copy()
    #
    #     """
    #     perform cross sectional regression upon the alpha
    #     """
    #
    #     if not latest_data.empty:
    #         latest_data.loc[:, 'industry_size_neutral_alpha'] = calculate_residuals(latest_data, 'log_size',
    #                                                                                 'industry_neutral_alpha')
    #         self.latest_data=latest_data
    #         daily_df.loc[daily_df['Date'] == latest_date, 'industry_size_neutral_alpha'] = latest_data[
    #             'industry_size_neutral_alpha']
    #     else:
    #         print('latest data is empty')
    #
    #     # Save the updated data
    #     self.pv_df = df
